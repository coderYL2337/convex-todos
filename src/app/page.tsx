  ...
  return (
    <div className="max-w-screen-md mx-auto p-4 space-y-4">
      ...
      {todos.map(({title, description, completed}, index) => (
        <ToDoItem  key={index} title={title} description={description} completed={completed} onCompleteChanged={newValue => {
          setTodos(prev =>{
            const newTodos = [...prev];
...
function ToDoItem({title, description, completed, onCompleteChanged, onRemove}: 
  ...
  return (
        <li className="w-full flex gap-2 items-center border rounded p-2">
          <input type="checkbox" checked={completed} onChange={e => onCompleteChanged(e.target.checked)} />
          <div>
